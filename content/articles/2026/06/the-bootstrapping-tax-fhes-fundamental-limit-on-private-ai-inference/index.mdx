...
artifacts: [the-fhe-inference-wall, the-noise-ceiling]
...
The noise budget makes the tradeoff concrete. Each multiplication spends down a finite budget of "levels"; when it runs out, the ciphertext must be bootstrapped — and the two schemes sit at opposite extremes of *when* that happens and *how much* each refresh costs. Drag the circuit deeper and watch the noise sawtooth: CKKS climbs for many operations then pays ~1 minute to refresh, while TFHE refreshes on every gate for ~1 millisecond.

<Artifact slug="the-noise-ceiling" />
