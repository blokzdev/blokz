...
artifacts: [the-focil-slot, the-exclusion-map]
...
<Artifact slug="the-exclusion-map" />

The map above places today's relays by inclusion policy and block-delivery share. The right half is the exclusion zone: for a transaction those relays screen out, expected inclusion runs ~1.6 slots and there is nothing to bid against. Toggle FOCIL to see what changes.
