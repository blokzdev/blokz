...
artifacts: [the-range-optimizer, the-liquidity-surface]
...
Seen whole, that tradeoff is a landscape. The surface below plots the two forces the paradox pits against each other over every (range width, volatility) pair — rebalancing cadence as height, fee APR as colour — both climbing toward the narrow-range edge. Slide volatility and watch the *human weekly-cadence frontier* move: the width below which a position rebalances more than once a week is agent-only territory, and it widens as volatility rises.

<Artifact slug="the-liquidity-surface" />
